...
public class FollowPlayer : MonoBehaviour
{
    ...
    //行走时摄像机 跟随的参数
    public float walk_distance_away;
    public float walk_distance_up;
    ...
    //到达终点的摄像机设置
    public float end_run_distance_away;
    public float end_run_distance_up;
    public float end_run_speed;

    public float end_walk_distance_away;
    public float end_walk_distance_up;
    public float end_walk_speed;
    ...
    void LateUpdate()
    {
        ...
        else if (Game_parameter.game_statu == Game_statu.success)
        {
            ...
            else if (this.animator.GetCurrentAnimatorStateInfo(0).IsName("walk") == true)
            {
                ...
                /*
                  targetPosition = player_transform.position + Vector3.up * end_walk_distance_up + player_transform.forward * end_walk_distance_away;
                transform.position = Vector3.Lerp(transform.position, targetPosition, Time.deltaTime * end_walk_speed);
                 */
